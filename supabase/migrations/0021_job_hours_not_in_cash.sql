-- Migration 0021 — job_hours sai do cálculo de caixa real
-- Aplicada em: 2026-05-18
--
-- Motivo:
--   job_hours hoje entra direto em v_finance_monthly e v_account_balance.
--   Mas o José paga funcionários toda sexta via business_expense payroll.
--   Resultado: double counting (horas contam quando lançadas E quando pagas).
--
-- Decisão (José 2026-05-18):
--   Opção A — job_hours NÃO entra no caixa. Só conta no caixa quando o José
--   clicar "Pagar tudo" (que cria business_expense payroll).
--
-- O que MUDA:
--   - v_finance_monthly: tira job_hours_cost do total_paid_out (mantém campo
--     pra info contextual, mas com valor 0 ou só pra leitura)
--   - v_account_balance: tira job_hours dos outflows_bank
--
-- O que NÃO muda:
--   - v_job_margin: continua somando job_hours como custo (margem precisa ver
--     o custo real do job, mesmo que ainda não pago).
--   - job_hours table: continua existindo, lançamento de horas continua igual.
--
-- =============================================================================
...
-- job_hours NÃO conta no caixa — entra só quando paga via business_expense payroll
job_hours_info AS (
...
ads_cash AS (
  SELECT month::date, SUM(amount) AS total_amount
  FROM ad_spend
  WHERE payment_method IS NULL OR payment_method != 'credit_card'
  GROUP BY 1
),
...
SELECT
  ...
  COALESCE(jh.total_amount, 0)::numeric AS job_hours_cost,  -- info contextual (não entra em total_paid_out)
  ...
  -- total_paid_out SEM job_hours (entra só via business_expense payroll quando pago)
...
-- =============================================================================
-- v_account_balance recriada SEM job_hours
-- =============================================================================
...
outflows_bank AS (
  ...
  -- NOTA: job_hours NÃO entra mais (conta só via business_expense payroll quando pago)
  UNION ALL
...
SELECT month_label, sold, received, total_paid_out, cash_balance FROM v_finance_monthly
