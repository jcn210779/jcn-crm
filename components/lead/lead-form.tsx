...
import { SERVICE_LABEL, SOURCE_LABEL, TARGET_CITIES } from "@/lib/labels";
...
type Props = {
  /** "create" cadastra um lead novo. "edit" atualiza um lead existente. */
  mode: "create" | "edit";
  /** Lead a editar (obrigatório quando mode="edit") pra pré-preencher. */
  lead?: Lead;
  /** Chamado após salvar com sucesso. Em "edit" fecha o dialog. Em "create"
   *  o default redireciona pro pipeline. */
  onSaved?: () => void;
  /** Em "edit", o form vive dentro de um dialog — esconde a moldura de
   *  card/header própria da página de cadastro. */
  embedded?: boolean;
};
...
function valuesFromLead(lead?: Lead): FormValues {
  return {
    ...
    phone: lead?.phone ?? "",
    city: lead?.city ?? "",
    service_interest: lead?.service_interest ?? "deck",
...
export function LeadForm({ mode, lead, onSaved, embedded = false }: Props) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const estimatedValue = values.estimated_value
    ...
    if (mode === "edit" && lead) {
      ...
      toast.success("Lead atualizado", { description: values.name });
      router.refresh();
      onSaved?.();
      return;
    }
      ...
      <section className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="Estado">
          <Input
            placeholder="MA"
            maxLength={2}
            className="h-11 border-white/10 bg-white/[0.03] uppercase"
            {...form.register("state")}
          />
        </Field>
  ...
  const submitLabel = mode === "edit" ? "Salvar alterações" : "Criar lead";

  // Modo embedded (dentro do dialog de edição): só os campos + footer simples.
  if (embedded) {
    return (
  ...
  // Modo página inteira (cadastro em /lead/novo): card + header.
  return (
    <form
